...
C First Iteration:
C
C The first iteration uses q = (src[0]-c) * inverse shown in the first
C style.  This lets us get the first q as soon as the inverse is ready,
C without going through si=s*inverse.  Basically at the start we have c and
C can use it while waiting for the inverse, whereas in the second and
C subsequent iterations it's the other way around, ie. we have the inverse
C and are waiting for c.
C
